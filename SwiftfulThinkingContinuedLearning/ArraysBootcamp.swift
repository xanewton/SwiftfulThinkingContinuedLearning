...
class ArrayModificationViewModel: ObservableObject {
    ...
    func updateFilteredArray() {
        ...
        // the compact map ignores nil elements
        /*mappedArray = dataArray.compactMap({ (user) -> String? in
            user.name
        })
        mappedArray = dataArray.compactMap({ $0.name })*/
        
        // Combine sort, filter, map
    ...
    func getUsers() {
        ...
        let user3 = UserModel(name: nil /*"Joe"*/, points: 10, isVerified: true)
        ...
        let user8 = UserModel(name: nil /*"Lisa"*/, points: 45, isVerified: true)
